...
public class MQTTCommunication {
  ...
  public void connect(boolean clearTopics) throws MqttException {
    ...
    for (var topic : QueueNameEnum.values()) {
      client.getTopic(topic.name());
      if (clearTopics)
        client.publish(topic.name(), new byte[0], 0, true);
    }
  }
